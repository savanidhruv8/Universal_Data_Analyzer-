...
def clean_data(df, imputation_methods=None, remove_outliers=False, outlier_method=None, encoding_method=None, 
    ...
    if encoding_method and encoding_column and encoding_column in df.columns:
    ...
    elif encoding_method and encoding_column and encoding_column not in df.columns:
        summary["encoded_columns"].append(f"{encoding_column} (Skipped: Column not found after cleaning)")

    # Apply user-specified data type conversions
    if dtype_conversions:
        for col, target_dtype in dtype_conversions.items():
            if col not in df.columns:
                summary["dtype_conversion_failed"].append(f"{col}: Column not found")
                continue
            try:
                if target_dtype == 'int':
                    if df[col].isnull().any():
...
if st.session_state.df is not None:
    ...
    for col in df.select_dtypes(include='object').columns:
        try:
            ...
            continue
    all_numeric_cols = [col for col in numeric_cols + potential_numeric_cols if col not in columns_to_remove]
    ...
    # Encoding options
    categorical_cols = [col for col in df.select_dtypes(include='object').columns if col not in columns_to_remove]
    encoding_column = None
    encoding_method = None
    if categorical_cols:
        encoding_column = st.selectbox("Select column for encoding", ["None"] + categorical_cols)
        if encoding_column != "None":
            encoding_method = st.radio("Encoding method", ["Label Encoding", "One-Hot Encoding"])
    else:
        st.write("No categorical columns available for encoding.")

    # Data type conversion options
    st.subheader("🔄 Data Type Conversion")
    dtype_conversions = {}
    available_columns = [col for col in df.columns if col not in columns_to_remove]
    num_conversions = st.number_input("Number of columns to convert", min_value=0, max_value=len(available_columns), step=1)
    for i in range(num_conversions):
        col = st.selectbox(f"Select column {i+1} for type conversion", available_columns, key=f"col_{i}")
    ...
    if st.button("🚀 Clean and Analyze Dataset"):
        ...
        elif algorithm_type != "None" and target_column and target_column not in cleaned_df.columns:
            st.error(f"Target column '{target_column}' not found in cleaned dataset.")